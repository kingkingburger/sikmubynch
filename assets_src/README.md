...
- 현재 상태(2026-09-22): **건물 8종 + 몬스터 4포즈 시트 4종 모두 완료.** `gun_tower.png`는 2D 카툰으로 교체했고, `rusher_sheet` `tank_sheet` `splitter_sheet` `mini_sheet`는 치비 버전으로 임포트해 게임에 적용했다. 낱장 `rusher.png` 등은 시트가 있으면 무시된다. 다시 만들 때만 `PROMPTS.md` A를 쓴다.
