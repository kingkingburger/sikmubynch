...
SIKMUBYNCH는 플레이어가 한 런 동안 본진을 지키며 자원을 모으고, 타워와 방어물을 배치하고, 점점 커지는 압박에 화력을 집중하는 생존 게임이다. 적은 쉬지 않고 흘러 들어오며, 그 위에 주기적인 급증이 겹친다. 플레이어는 방어선을 임시로 막고, 화력을 집중하고, 무너진 곳을 다시 세우며 버틴다. 웨이브 사이 휴식은 없다.

## 핵심 경험

- **끊기지 않는 압박**: 초반부터 적 무리가 보이고, 중후반에는 수천 단위의 호드가 방어선을 밀어붙인다. 스트림은 멈추지 않고, 급증(밀도·돌파·폭풍)이 그 위에 겹친다.
...
- [게임 디자인](docs/game-design.md): 첫 프로토타입 범위, 핵심 루프, 스트림·급증 압박, 건물·적, 보류 시스템
...
| 스크립트 | 실행 방식 | 다루는 것 |
| --- | --- | --- |
| `tools/tests/gameplay_regression.gd` | headless | 시작 상태, Flow Field, 배치·철거·기본 수입, 본진 도달·공격, 타워 처치·분열체, 압박이 쉬지 않음(5초 창마다 스폰), 급증 일정·시간 배율, 결정론(같은 seed 같은 해시), 봉쇄 돌파, 포격 광역·감속, 화염·전격·저격, 공격자 상한·본진 재생, 500마리 틱 예산 |
| `tools/tests/play_smoke.gd` | headless / 창 모드 | 게임 씬 로드, MultiMesh 버퍼, 씬을 통한 배치·철거, 화면→타일, 일시정지·ESC·속도, 게임오버·재시작, 500마리 렌더 |
| `tools/tests/soak_waves.gd` | headless | 스크립트 방어선으로 N번째 급증(`SOAK_WAVES`)까지 돌리며 급증별 규모·틱 시간·본진 HP 추세 기록 |
...
│   ├── wave_sim.gd          # 상시 스트림 + 급증(밀도·돌파·폭풍), 시간 기반 배율, 스폰 큐
