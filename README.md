...
- **2026-09-21 코드 전환**: 3D 씬·GLB 모델·보류 시스템 코드를 제거하고, `project/sim`(Node 없는 시뮬레이션)과 `project/render`(MultiMesh 2D 렌더러)로 재구성했다. headless 소크에서 800~1,000마리 동시 생존 시 시뮬레이션 틱 평균 0.25 ms, 창 모드 895마리 렌더 약 1 ms를 확인했다. Flow Field 재계산(건물 배치·파괴 시) 30~40 ms 히치는 2026-09-25에 여러 틱으로 나눠 계산하도록 바꿔 틱당 약 1.5 ms로 줄였다.
...
- **2026-09-22 두 번째 플레이 반영**: “중간에 몰아치지 않는다, 웨이브 개념이 없어야 한다”는 판단으로 급증(카운트다운·배너·덩어리)을 없앴다. 압박은 한 흐름의 스트림이며 배율과 방향 가중치가 예고 없이 천천히 떠돈다. 소크 8분: 동시 적 수가 골 없이 36→300. 이어서 “여전히 적다”는 판단으로 살아 있는 적을 시간에 따라 오르는 목표 무리 수(5분 560, 10분 1,460)까지 채우는 디렉터를 넣었다 — 처치 보상 경제에서는 방어력이 누적 처치에 비례해 커져 시간 곡선만으로는 화면이 비기 때문이다. 같은 날 2D 카툰 아트 세트(건물 8종·치비 몬스터 4포즈 시트 4종)를 전부 적용했다.
