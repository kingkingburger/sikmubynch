...
tools/                                 # 회귀 검증 실행기, import_sprites.py(원본→스프라이트), tests/ (headless 회귀·씬 스모크·소크·물량 스트레스·스크린샷)
...
- 2026-09-25 단계 D 착수 — Flow Field 재계산 분산: 뒤쪽 버퍼에 틱마다 셀 예산만큼 계산 후 교체. 건설 직후 히치 37 ms → 틱당 약 1.5 ms(약 0.4초에 걸쳐 적용). 측정 기록은 `docs/technical-design.md` “측정 기록” 표에 누적한다.
- 2026-09-25 물량 단계 측정: 개발 PC 창 모드 1,000 243fps / 5,000 112fps / 10,000 72fps, 틱 평균 10,000에서 7.8 ms. 단계 D 성능 목표는 이미 충족. 적 상한 12,288, 렌더 버퍼는 물량에 맞춰 성장. 측정 도구 `tools/tests/stress_scale.gd`(headless)·`render_stress.gd`(창 모드).
