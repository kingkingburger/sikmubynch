...
- **첫 프로토타입 범위**: 맵 1개, 본진 1개, 방어물 2종, 타워 6종(첫 플레이 후 3종에서 확장), 적 3종, 자원 1종, 끊기지 않는 압박(스트림 + 급증, 웨이브 휴식 없음). 그 외는 없다. 유닛·배럭·채굴기·버프 타워도 첫 범위에 들어가지 않는다.
...
- 압박은 끊기지 않는다. 스트림이 항상 흐르고 급증(밀도·돌파·폭풍)이 그 위에 겹친다. 웨이브 사이 휴식은 없다.
...
- **WaveSim**: 상시 스트림 스폰율, 급증(밀도·돌파·폭풍) 규모·방향, 시간 기반 배율, 스폰 큐
